...
@interface TweetyViewController ()
...
@implementation TweetyViewController
...
-(void)labelSwitchedAtIndex:(NSUInteger)index{
    ...
    NSLog(@"index:%d",index);
    _headerText = self.navigationTitles[self.selectedLabelIndex];
...
- (void)viewDidLoad
{
    ...
    self.delegate = self;
    ConnectionManager *connectionManager = [ConnectionManager sharedInstanse];
    
    [connectionManager startUpdateLocation];
    NSArray *array = [TWSValidator getArrayWithObject:[connectionManager activeAddresses]];
    self.navigationTitles = [[NSMutableArray alloc] initWithArray:array];
